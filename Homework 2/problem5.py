...
from quicksort_median_of_3_pivot import quicksort_median_pivot
...
def main():
    ...
    for size in tqdm(sizes):
        ...
        for _ in range(5):  # run each algorithm 5 times
            ...
            y = x[1]
            start = time.perf_counter()
            quicksort_median_pivot(B, y, len(B) - 1)
            end = time.perf_counter()
    ...
    plt.plot(sizes, qs_rpivot_times, label='Heap Sort')
    plt.plot(sizes, qs_median_three_times, label='Quick Sort Rightmost Pivot')
